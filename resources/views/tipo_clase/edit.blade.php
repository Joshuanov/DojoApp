<x-app-layout>
    ...
    <div class="py-6 px-4 max-w-3xl mx-auto sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('tipo_clase.update', $tipo_clase) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <x-text-input id="nombre_clase" name="nombre_clase" type="text" class="mt-1 block w-full"
                    value="{{ old('nombre_clase', $tipo_clase->nombre_clase) }}" required />
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion"
                    class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('descripcion', $tipo_clase->descripcion) }}</textarea>
            ...
            <div>
                ...
                <x-text-input id="duracion" name="duracion" type="number" class="mt-1 block w-full"
                    value="{{ old('duracion', $tipo_clase->duracion) }}" min="1" required />
            ...
            <div>
                ...
                <select id="grupo" name="grupo"
                    ...
                    @foreach(['Tiger', 'Junior', 'Senior', 'Adultos', 'Sanda', 'Seminario'] as $grupo)
                        <option value="{{ $grupo }}" @selected(old('grupo', $tipo_clase->grupo) === $grupo)>
                            {{ $grupo }}
                        </option>
                    @endforeach
                </select>
